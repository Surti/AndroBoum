    ...
    @Override
        protected void onCreate(Bundle savedInstanceState) {
            ...
            if (auth.getCurrentUser() != null) {
                // déjà connecté
                Log.v("AndroBoum", "je suis dé jà connecté sous l 'email :" + auth.getCurrentUser().getEmail());
                setUser();
                updateProfil(user);
                downloadImage();
                ...
                startActivityForResult(AuthUI.getInstance().createSignInIntentBuilder().setAvailableProviders(Arrays.asList(
                        ...
                        new AuthUI.IdpConfig.Builder(AuthUI.FACEBOOK_PROVIDER).build())).build(),123);
            }
        ...
        Button bouton = (Button) findViewById(R.id.BoutonList);
        bouton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                lancerUserListActivity();
            }
        });
